/*
 * verify_lkf_vector.c  —  Milestone-4 Verification Harness (Vectorised LKF)
 * ==========================================================================
 * Compares the RVV-vectorised LKF (lkf_vector.s) against the Python reference
 * (lkf_results.csv) using the same §6 methodology as Milestone-3.
 *
 ...
 *   riscv64-linux-gnu-gcc -march=rv64gcv -mabi=lp64d -O0 -g -Wall \
 *       verify_lkf_vector.c lkf_vector.o ekf_utils_vector.o \
 *       matrix_vec.o matrix_asm.o \
 *       -o verify_lkf_vector -lm -static
 *
 * Run:
 *   qemu-riscv64 -cpu rv64,v=true,vlen=128 \
 *       ./verify_lkf_vector "<noisy_csv>" "<ref_lkf_csv>" [max_frames]
 *
 * ==========================================================================
 */
...
/* ---- vectorised LKF assembly function declarations ----------------------- */
...
extern void   lkf_vec_get_positions     (const void *lkf, double *pos_out);
...
int main(int argc, char **argv)
{
    if (argc < 3) {
        fprintf(stderr,
            "Usage: %s <noisy_csv> <ref_lkf_csv> [max_frames]\n",
            argv[0]);
        return 1;
    }
    ...
    /* ---- Allocate and initialise vectorised LKF --------------------------- */
    size_t sz = lkf_vec_sizeof();
    printf("[INFO] LKF-VEC struct: %zu bytes (%.2f MB)\n",
           sz, (double)sz / (1024.0 * 1024.0));

    /* Use aligned allocation for cache-line efficiency with RVV */
    void *lkf = NULL;
    if (posix_memalign(&lkf, 64, sz) != 0 || !lkf) {
    ...
    for (int j = 0; j < NUM_JOINTS; j++) {
        ...
        lkf_vec_set_initial_state(lkf, j, p3);
    }
    ...
    /* ---- Main filter loop ------------------------------------------------- */
    printf("[INFO] Running LKF-VEC (RVV vectorised) on %d frames...\n", N);
    double t_start = now_sec();

    for (int fr = 0; fr < N; fr++) {
        if (fr > 0) lkf_vec_predict(lkf);
        ...
        lkf_vec_update(lkf, z);
        lkf_vec_get_full_state(lkf, results[fr]);

        if ((fr + 1) % PROGRESS_INTERVAL == 0 || fr == N - 1)
            printf("\r  LKF-VEC: %d/%d   ", fr + 1, N);
    }
    double elapsed = now_sec() - t_start;
    printf("\n[INFO] LKF-VEC loop done.  Total time: %.3f s  (%.1f ms/frame)\n",
           elapsed, elapsed * 1000.0 / N);

    /* ---- Save results CSV ------------------------------------------------- */
    {
        FILE *out = fopen("lkf_vec_results.csv", "w");
        fprintf(out, "frame");
        for (int j = 0; j < NUM_JOINTS; j++)
        ...
        fclose(out);
        printf("[OK] Saved: lkf_vec_results.csv\n");
    }
    ...
    /* ---- Print summary --------------------------------------------------- */
    printf("\n==================================================================\n");
    printf("  Milestone-4 Numerical Verification  (LKF-VEC, RVV vectorised)\n");
    printf("==================================================================\n");
    printf("  Tolerance          : %.1e\n",  EPSILON_TOL);
    ...
    printf("\n  [%s] LKF-VEC Milestone-4 Verification\n",
           violations == 0 ? "PASS \xe2\x9c\x93" : "FAIL \xe2\x9c\x97");
    printf("==================================================================\n\n");

    /* ---- Save verification CSV ------------------------------------------- */
    {
        FILE *v = fopen("lkf_vec_verification.csv", "w");
        ...
        fclose(v);
        printf("[OK] Saved: lkf_vec_verification.csv\n");
    }
