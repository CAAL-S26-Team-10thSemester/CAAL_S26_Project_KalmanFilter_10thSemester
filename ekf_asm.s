# =============================================================================
#  ekf_asm.s  —  RISC-V Scalar Assembly: Extended Kalman Filter
#  Kalman Filter Milestone-3
# =============================================================================
#
#  Implements the EKF class from kalman-updated.py exactly:
#
#    ekf_sizeof            ()                    — return heap size needed
#    ekf_init              (ekf, dt)             — __init__
#    ekf_set_initial_state (ekf, joint, pos3)    — set_initial_state
#    ekf_predict           (ekf)                 — predict  (identical to LKF)
#    ekf_update            (ekf, z_cart_flat)    — update   (spherical model)
#    ekf_get_positions     (ekf, pos_out)        — get_positions
#    ekf_get_full_state    (ekf, out)            — get_full_state
#
#  EKF-specific helpers (internal, not exported to C):
#    ekf_compute_h         (x, h_out)            — nonlinear measurement h(x)
#    ekf_compute_jacobian  (x, Hk_out)           — Jacobian dh/dx (69x276)
#    ekf_init_R            (R)                   — build spherical noise R
#
# =============================================================================
#  DESIGN DECISIONS
#
...
#     for large buffers.  ekf_sizeof returns 5149752 (different from LKF).
#     The C driver must use ekf_sizeof() to allocate — do not hard-code.
#
...
#     ekf_predict uses jscratch sub-regions for Ftmp, FT and x_new so that
...
# =============================================================================
#  ekf_sizeof  —  return total allocation size
#  size_t ekf_sizeof(void)
#  Leaf.
# =============================================================================
    .globl ekf_sizeof
    .type  ekf_sizeof, @function
ekf_sizeof:
    la      t0, ekf_TOTAL_BYTES
    ld      a0, 0(t0)
    ret
    .size ekf_sizeof, .-ekf_sizeof


# =============================================================================
#  ekf_init_R  —  build R (69x69 spherical measurement noise, diagonal)
#  void ekf_init_R(double *R)   a0=R
# =============================================================================
    .globl ekf_init_R
    .type  ekf_init_R, @function
ekf_init_R:
    addi    sp, sp, -24
    ...
    # Zero R: 69*69*8 = 38088 bytes — unrolled x8
    li      t0, 69; mul t0, t0, t0; slli t0, t0, 3
    add     t1, s0, t0
    la      t2, ekf_fp_zero; fld ft0, 0(t2)
    mv      t2, s0
    addi    t3, t1, -56
.LekfR_zero_u:
    bgt     t2, t3, .LekfR_zero_t
    fsd     ft0,  0(t2); fsd ft0,  8(t2); fsd ft0, 16(t2); fsd ft0, 24(t2)
    fsd     ft0, 32(t2); fsd ft0, 40(t2); fsd ft0, 48(t2); fsd ft0, 56(t2)
    addi    t2, t2, 64; j .LekfR_zero_u
.LekfR_zero_t:
    bge     t2, t1, .LekfR_zero_done
    fsd     ft0, 0(t2); addi t2, t2, 8; j .LekfR_zero_t
.LekfR_zero_done:
...
.LekfR_done:
    ...
    addi    sp, sp, 24; ret
    .size ekf_init_R, .-ekf_init_R


# =============================================================================
#  ekf_init  —  initialise all fields of an EKF struct
#  void ekf_init(void *ekf, double dt)   a0=ekf  fa0=dt
# =============================================================================
    .extern state_init_F
    .extern state_init_Q

    .globl ekf_init
    .type  ekf_init, @function
ekf_init:
    addi    sp, sp, -24
    ...
    # x = zeros(276): 2208 bytes — unrolled x4
    la      t0, ekf_fp_zero; fld ft0, 0(t0)
    addi    t1, s0, 8                           # &x[0]
    li      t2, 2208; add t2, t1, t2
    addi    t3, t2, -24
.Lekfinit_xz_u:
    bgt     t1, t3, .Lekfinit_xz_t
    fsd     ft0,  0(t1); fsd ft0,  8(t1); fsd ft0, 16(t1); fsd ft0, 24(t1)
    addi    t1, t1, 32; j .Lekfinit_xz_u
.Lekfinit_xz_t:
    bge     t1, t2, .Lekfinit_xz_done
    fsd     ft0, 0(t1); addi t1, t1, 8; j .Lekfinit_xz_t
.Lekfinit_xz_done:
    ...
    addi    sp, sp, 24; ret
    .size ekf_init, .-ekf_init


# =============================================================================
#  ekf_set_initial_state
#  void ekf_set_initial_state(void *ekf, int joint_idx, double *pos3)
#  a0=ekf  a1=joint_idx  a2=pos3   Leaf.
# =============================================================================
    .globl ekf_set_initial_state
    .type  ekf_set_initial_state, @function
ekf_set_initial_state:
    ...
    ret
    .size ekf_set_initial_state, .-ekf_set_initial_state


# =============================================================================
#  ekf_predict  —  prediction step
#
...
#
#  void ekf_predict(void *ekf)   a0=ekf
#
#  Register allocation:
...
ekf_predict:
    ...
    # copy x_new -> x  (unrolled x4)
    # s1, s7 still valid (callee-saved)
    mv      t2, s1; mv t3, s7
    li      t4, 2208; add t4, t2, t4
    addi    t5, t4, -24
.Lekfpr_xcopy_u:
    bgt     t2, t5, .Lekfpr_xcopy_t
    fld     ft0,  0(t3); fld ft1,  8(t3); fld ft2, 16(t3); fld ft3, 24(t3)
    fsd     ft0,  0(t2); fsd ft1,  8(t2); fsd ft2, 16(t2); fsd ft3, 24(t2)
    addi    t2, t2, 32; addi t3, t3, 32; j .Lekfpr_xcopy_u
.Lekfpr_xcopy_t:
    bge     t2, t4, .Lekfpr_xcopy_done
    fld     ft0, 0(t3); fsd ft0, 0(t2)
    addi    t2, t2, 8; addi t3, t3, 8; j .Lekfpr_xcopy_t
.Lekfpr_xcopy_done:
...
# =============================================================================
#  ekf_compute_h  —  nonlinear measurement h: R^276 -> R^69
...
.Leh_done:
    ...
    addi    sp, sp, 48; ret
    .size ekf_compute_h, .-ekf_compute_h


# =============================================================================
#  ekf_compute_jacobian  —  Hk = dh/dx  (69x276)
...
# =============================================================================
    ...
    .type  ekf_compute_jacobian, @function
ekf_compute_jacobian:
    ...
    # Zero Hk: 69*276*8 = 152352 bytes — unrolled x8
    ...
    addi    t3, t1, -56
.Lej_zero_u:
    bgt     t2, t3, .Lej_zero_t
    fsd     ft0,  0(t2); fsd ft0,  8(t2); fsd ft0, 16(t2); fsd ft0, 24(t2)
    fsd     ft0, 32(t2); fsd ft0, 40(t2); fsd ft0, 48(t2); fsd ft0, 56(t2)
    addi    t2, t2, 64; j .Lej_zero_u
.Lej_zero_t:
    bge     t2, t1, .Lej_zero_done
    fsd     ft0, 0(t2); addi t2, t2, 8; j .Lej_zero_t
.Lej_zero_done:
...
.Lej_done:
    ...
    addi    sp, sp, 40; ret
    .size ekf_compute_jacobian, .-ekf_compute_jacobian


# =============================================================================
#  ekf_update  —  EKF measurement update (spherical model)
#
...
#
#  void ekf_update(void *ekf, const double *z_cart_flat)
...
ekf_update:
    ...
    # Zero z_tmp (2208 bytes) — unrolled x8
    la      t3, ekf_fp_zero; fld ft0, 0(t3)
    mv      t3, t2
    li      t4, 2208; add t4, t2, t4
    addi    t5, t4, -56
.Lupd_ztmp_zero_u:
    bgt     t3, t5, .Lupd_ztmp_zero_t
    fsd     ft0,  0(t3); fsd ft0,  8(t3); fsd ft0, 16(t3); fsd ft0, 24(t3)
    fsd     ft0, 32(t3); fsd ft0, 40(t3); fsd ft0, 48(t3); fsd ft0, 56(t3)
    addi    t3, t3, 64; j .Lupd_ztmp_zero_u
.Lupd_ztmp_zero_t:
    bge     t3, t4, .Lupd_ztmp_zero_done
    fsd     ft0, 0(t3); addi t3, t3, 8; j .Lupd_ztmp_zero_t
.Lupd_ztmp_zero_done:
...
.Lupd_fill_ztmp_done:
    ...
    # -----------------------------------------------------------------------
    # Step 2: z_sph = ekf_compute_h(z_tmp)
    # -----------------------------------------------------------------------
    la      t0, ekf_OFF_jscratch; ld t0, 0(t0); add t0, s0, t0
...
.Lupd_nu_done:
    ...
    # -----------------------------------------------------------------------
    # Step 5: Hk = ekf_compute_jacobian(x)  -> s10 (jscratch + JS_Hk)
    # -----------------------------------------------------------------------
    mv      a0, s1; mv a1, s10
    call    ekf_compute_jacobian
...
.Lupd_ekf_ok:
    ...
    call    mat_mul   # K_tmp = PHt @ Sinv

    # Copy K_tmp -> PHt buffer (s4) — unrolled x4
    la      t0, ekf_OFF_jscratch; ld t0, 0(t0); add t0, s0, t0
    la      t1, ekf_JS_Ktmp;      ld t1, 0(t1); add t3, t0, t1   # t3 = K_tmp src
    mv      t2, s4                          # dst = PHt buffer = K
    li      t4, 152352; add t4, t2, t4
    addi    t5, t4, -24
.Lupd_kcopy_u:
    bgt     t2, t5, .Lupd_kcopy_t
    fld     ft0,  0(t3); fld ft1,  8(t3); fld ft2, 16(t3); fld ft3, 24(t3)
    fsd     ft0,  0(t2); fsd ft1,  8(t2); fsd ft2, 16(t2); fsd ft3, 24(t2)
    addi    t2, t2, 32; addi t3, t3, 32; j .Lupd_kcopy_u
.Lupd_kcopy_t:
    bge     t2, t4, .Lupd_kcopy_done
    fld     ft0, 0(t3); fsd ft0, 0(t2)
    addi    t2, t2, 8; addi t3, t3, 8; j .Lupd_kcopy_t
.Lupd_kcopy_done:
    # s4 now = K (276x69)
...
# =============================================================================
#  ekf_get_positions
#  void ekf_get_positions(const void *ekf, double *pos_out)
#  a0=ekf  a1=pos_out (double[69])   Leaf.
# =============================================================================
    .globl ekf_get_positions
    .type  ekf_get_positions, @function
ekf_get_positions:
    la      t0, ekf_OFF_x; ld t0, 0(t0); add t0, a0, t0
    li      t1, 0
.Lekfgp_j:
...
.Lekfgp_done:
    ret
    .size ekf_get_positions, .-ekf_get_positions


# =============================================================================
#  ekf_get_full_state
#  void ekf_get_full_state(const void *ekf, double *out)
#  a0=ekf  a1=out (double[276])   Leaf.  Unrolled x4.
# =============================================================================
    .globl ekf_get_full_state
    .type  ekf_get_full_state, @function
ekf_get_full_state:
    la      t0, ekf_OFF_x; ld t0, 0(t0); add t0, a0, t0
    li      t1, 2208; add t1, t0, t1
    addi    t2, t1, -32
.Lekfgfs_u:
    bgt     t0, t2, .Lekfgfs_t
    fld     ft0,  0(t0); fld ft1,  8(t0); fld ft2, 16(t0); fld ft3, 24(t0)
    fsd     ft0,  0(a1); fsd ft1,  8(a1); fsd ft2, 16(a1); fsd ft3, 24(a1)
    addi    t0, t0, 32; addi a1, a1, 32; j .Lekfgfs_u
.Lekfgfs_t:
    bge     t0, t1, .Lekfgfs_done
    fld     ft0, 0(t0); fsd ft0, 0(a1)
    addi    t0, t0, 8; addi a1, a1, 8; j .Lekfgfs_t
.Lekfgfs_done:
    ret
    .size ekf_get_full_state, .-ekf_get_full_state
