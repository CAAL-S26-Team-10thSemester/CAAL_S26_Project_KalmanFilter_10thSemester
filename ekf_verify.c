/*
 * ekf_verify.c  —  Milestone-3 §6 Numerical Verification Harness (EKF)
 * ======================================================================
 * Mirrors lkf_verify.c structure exactly:
 *   • Per-joint average absolute error table  (Table A)
 *   • Per-component average absolute error table  (Table B)
 *   • Global max and min error
 *   • Violation count vs ε_tol = 1e-9
 *   • Saves ekf_asm_results.csv and ekf_asm_verification.csv
 *
 * Build (via Makefile target 'ekf'):
 *   riscv64-linux-gnu-gcc -march=rv64imfd -mabi=lp64d -O0 -g -Wall \
 *       ekf_verify.c ekf_asm.o lkf_asm.o matrix_asm.o \
 *       -o ekf_verify_bin -lm -static
 *
 * Run:
 *   qemu-riscv64 ./ekf_verify_bin <noisy_csv> <ref_ekf_csv> [max_frames]
 *
 * Where ref_ekf_csv is the output of kalman-updated.py (Python reference).
 * ======================================================================
 */
...
int main(int argc, char **argv)
{
    ...
    size_t sz = ekf_sizeof();
    printf("[INFO] EKF struct: %zu bytes (%.2f MB)\n",
           sz, (double)sz / (1024.0 * 1024.0));

    void *ekf = malloc(sz);
    if (!ekf) { fprintf(stderr, "OOM\n"); return 1; }
    memset(ekf, 0, sz);
    ...
    for (int j = 0; j < NUM_JOINTS; j++) {
        ...
        ekf_set_initial_state(ekf, j, p3);
    }
    ...
    /* ---- Main filter loop ------------------------------------------------- */
    printf("[INFO] Running EKF assembly on %d frames...\n", N);
    double t_start = now_sec();

    for (int fr = 0; fr < N; fr++) {
        if (fr > 0) ekf_predict(ekf);
        ...
        ekf_update(ekf, z);
        ekf_get_full_state(ekf, results[fr]);

        if ((fr + 1) % PROGRESS_INTERVAL == 0 || fr == N - 1)
            printf("\r  EKF-ASM: %d/%d   ", fr + 1, N);
    }
    printf("\n[INFO] EKF loop done.  Total time: %.1f s\n", now_sec() - t_start);
    ...
    /* ---- Save results CSV ------------------------------------------------- */
    {
        ...
        fclose(out);
        printf("[OK] Saved: ekf_asm_results.csv\n");
    }

    /* =========================================================================
     * §6 Numerical Verification
     *   For each frame k and state component i:
     *     |x_asm[k,i] - x_ref[k,i]| <= epsilon_tol = 1e-9
     *
     *   Table A: avg |error| per joint (averaged over all frames and
     *            the STATE_DIM components of that joint)
     *   Table B: avg |error| per state component (averaged over all
     *            frames and all NUM_JOINTS joints)
     *   Also report global max and min |error|.
     * ========================================================================= */
    ...
    /* Normalise:
     *   avg_joint[j] / (N_frames * STATE_DIM) — avg over time and components
     *   avg_comp[c]  / (N_frames * NUM_JOINTS) — avg over time and joints    */
    ...
    /* ---- Print summary --------------------------------------------------- */
    printf("\n==================================================================\n");
    printf("  Milestone-3 §6 Numerical Verification  (EKF Assembly)\n");
    printf("==================================================================\n");
    printf("  Tolerance          : %.1e\n",  EPSILON_TOL);
    ...
    printf("\n  [%s] EKF Assembly Verification\n",
           violations == 0 ? "PASS ✓" : "FAIL ✗");
    printf("==================================================================\n\n");

    /* ---- Save verification CSV ------------------------------------------- */
    {
        FILE *v = fopen("ekf_asm_verification.csv", "w");
        ...
        fprintf(v, "summary,result,%s\n",
                violations == 0 ? "PASS" : "FAIL");
        fclose(v);
        printf("[OK] Saved: ekf_asm_verification.csv\n");
    }
